...
parser.add_argument('-g', '--generate', help="determines the amount of steps and thus determines dt(dt = tf/N)", action="store_true")
...
n = args.n
up = 2 # 0 = x, 1=y, 2=z is the altitude
...
max_vel.value = 1000
cos_glide_slope = cp.Parameter(1, name="cos_glide_slope", sign="positive") # equal to cosine of glideslope angle
cos_glide_slope.value = np.cos(np.radians(45))
...
max_angle.value = np.cos(np.radians(90))
...
for i in range(n):
    ...
    constraints.append(x[i, up] >= cp.norm(x[i, :3]) * cos_glide_slope) # makes sure the spacecraft doesn't go subsurface/hit terrain by defining a cone in which the spacecraft is allowed to fly
    ...
    if i != n - 1:
        acc = (g + (u[i+1, :] + u[i, :])/2)*dt 
        constraints += [
        x[i+1, :3] == x[i, :3] + (x[i, 3:] + x[i+1, 3:]) * dt / 2 - (u[i+1, :] - u[i, :])*dt_squared * (1/12), # position at t+1
        x[i+1, 3:] == x[i, 3:] + acc, # velocity at t+1
        z[i+1] == z[i] - a * (s[i] + s[i+1]) / 2 # update mass
        ]
